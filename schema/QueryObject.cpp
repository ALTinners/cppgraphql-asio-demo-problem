...
service::ResolverMap Query::getResolvers() const noexcept
{
	return {
		...
		{ R"gql(statusCode)gql"sv, [this](service::ResolverParams&& params) { return resolveStatusCode(std::move(params)); } }
	};
}
...
service::AwaitableResolver Query::resolveStatusCode(service::ResolverParams&& params) const
{
	auto argAddress = service::ModifiedArgument<std::string>::require("address", params.arguments);
	std::unique_lock resolverLock(_resolverMutex);
	auto directives = std::move(params.fieldDirectives);
	auto result = _pimpl->getStatusCode(service::FieldParams(service::SelectionSetParams{ params }, std::move(directives)), std::move(argAddress));
	resolverLock.unlock();
...
{
	typeQuery->AddFields({
		schema::Field::Make(R"gql(statusCode)gql"sv, R"md()md"sv, std::nullopt, schema->LookupType(R"gql(Int)gql"sv), {
			schema::InputValue::Make(R"gql(address)gql"sv, R"md()md"sv, schema->WrapType(introspection::TypeKind::NON_NULL, schema->LookupType(R"gql(String)gql"sv)), R"gql()gql"sv)
		})
	});
}
